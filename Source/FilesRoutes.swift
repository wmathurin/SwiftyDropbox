...
open class FilesRoutes {
    ...
    open func uploadSessionStart(_ close: Bool = false, input: Data) -> UploadRequest<Files.UploadSessionStartResultSerializer, VoidSerializer> {
    ...
    ///  - returns: Through the response callback, the caller will receive a `Files.UploadSessionStartResult` object on
    /// success or a `Void` object on failure.
    open func uploadSessionStart(_ close: Bool = false, input: URL) -> UploadRequest<Files.UploadSessionStartResultSerializer, VoidSerializer> {
        let route = Files.uploadSessionStart
        let serverArgs = Files.UploadSessionStartArg(close: close)
        return client.request(route, serverArgs: serverArgs, input: .file(input))
    }
    ...
    ///  - returns: Through the response callback, the caller will receive a `Files.UploadSessionStartResult` object on
    /// success or a `Void` object on failure.
    open func uploadSessionStart(_ close: Bool = false, input: InputStream) -> UploadRequest<Files.UploadSessionStartResultSerializer, VoidSerializer> {
        let route = Files.uploadSessionStart
        let serverArgs = Files.UploadSessionStartArg(close: close)
        return client.request(route, serverArgs: serverArgs, input: .stream(input))
    }
